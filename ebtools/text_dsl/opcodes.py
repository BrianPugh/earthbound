...
class OpcodeSpec:
    ...
    def __hash__(self):
        return hash((self.yaml_name, tuple(self.bytes)))
...
OPCODES: list[OpcodeSpec] = [
    ...
    OpcodeSpec("inflict_status", [0x19, 0x05], [_a("party_member", ArgType.PARTY), _a("status_group", ArgType.U8), _a("status", ArgType.U8)]),
...
def _build_lookup_by_bytes() -> dict[tuple[int, ...], OpcodeSpec]:
    ...
    for op in OPCODES:
        key = tuple(op.bytes)
        if key in result:
            raise ValueError(f"Duplicate byte sequence: {key} ({op.yaml_name} vs {result[key].yaml_name})")
        result[key] = op
    return result
