...
/* ESYSTEM text block SNES addresses (from earthbound.yml ESYSTEM offsets).
 * ESYSTEM base = ROM 0x7C588 → SNES 0xC7C588.
 * Addresses verified against build/earthbound.map. */
#define SNES_ESYSTEM_BASE        0xC7C588u
#define SNES_MSG_SYS_HANASU_NG   (SNES_ESYSTEM_BASE + 0x0000u) /* "Nothing happened" (talk fail) */
#define SNES_MSG_SYS_NOPROBLEM   (SNES_ESYSTEM_BASE + 0x0016u) /* "Nothing problem here." (check fail) */
#define SNES_MSG_SYS_GOODS_DROP  0xC7C609u  /* "Drop item?" confirmation text */
#define SNES_MSG_SYS_GOODS_USE_NG      0xC7C6B6u  /* "Nothing happened." (item use fallback) */
#define SNES_MSG_SYS_GOODS_NOCARRY     0xC7C6C9u  /* "can't carry this" */
#define SNES_MSG_SYS_GOODS_USE_NG_HERE 0xC7C6F1u  /* "It won't work here." */
#define SNES_MSG_SYS_GOODS_EQUIP       0xC7C742u  /* "Equip it to whom?" */
#define SNES_MSG_SYS_BICYCLE_ATARI_HERE 0xC7C833u /* "Can't ride bicycle here." */
#define SNES_MSG_SYS_TLPT_NG      0xC7C850u  /* "You can't use Teleport here." */
#define SNES_MSG_SYS_GOODS_USE_NG_USER 0xC77EE8u  /* "This person can't use this." */

/* Give text SNES addresses — 10 cases indexed by status combination.
 * Assembly indexes these via a jump table (lines 438-456 of open_menu.asm). */
#define SNES_MSG_SYS_CARRY_SELF_ALIVE               0xC7E3FAu  /* case 0: self, alive */
#define SNES_MSG_SYS_CARRY_FAIL_OTHER_ALIVE_ALIVE    0xC7E42Cu  /* case 1: no space, both alive */
#define SNES_MSG_SYS_CARRY_FAIL_OTHER_ALIVE_DEAD     0xC7E468u  /* case 2: no space, alive→dead */
#define SNES_MSG_SYS_CARRY_OTHER_ALIVE_ALIVE         0xC7E4A4u  /* case 3: success, both alive */
#define SNES_MSG_SYS_CARRY_OTHER_ALIVE_DEAD          0xC7E4C3u  /* case 4: success, alive→dead */
#define SNES_MSG_SYS_CARRY_SELF_DEAD                 0xC7E4E9u  /* case 5: self, dead */
#define SNES_MSG_SYS_CARRY_FAIL_OTHER_DEAD_ALIVE     0xC7E51Cu  /* case 6: no space, dead→alive */
#define SNES_MSG_SYS_CARRY_FAIL_OTHER_DEAD_DEAD      0xC7E559u  /* case 7: no space, both dead */
#define SNES_MSG_SYS_CARRY_OTHER_DEAD_ALIVE          0xC7E5A1u  /* case 8: success, dead→alive */
#define SNES_MSG_SYS_CARRY_OTHER_DEAD_DEAD           0xC7E5C2u  /* case 9: success, both dead */
        ...
        for (;;) {
            ...
            if (psi_category == PSI_CAT_OTHER) {
                ...
                if (blocked) {
                    ...
                    create_window(WINDOW_TEXT_BATTLE);
                    display_text_from_snes_addr(SNES_MSG_SYS_TLPT_NG);
                    close_focus_window();
                    result = 0;
                    goto handle_result;
                }
...
handle_result:
            ...
            if (battle_action_table) {
                ...
                if (desc_addr != 0)
                    display_text_from_snes_addr(desc_addr);
            }
...
static uint16_t overworld_use_item(uint16_t char_id, uint16_t item_slot) {
    ...
    if (type_category == 0) {
        ...
        desc_text_addr = SNES_MSG_SYS_GOODS_EQUIP;
        ...
        if (!check_item_usable_by(char_id, item_id)) {
            ...
            desc_text_addr = SNES_MSG_SYS_GOODS_USE_NG_USER;
        ...
        if (context_bits == 0x00) {
            ...
            desc_text_addr = SNES_MSG_SYS_GOODS_USE_NG_HERE;
            ...
            if (sub_type == 0 || sub_type == 1) {
                ...
                if (sector_item_type != item_id) {
                    ...
                    desc_text_addr = SNES_MSG_SYS_BICYCLE_ATARI_HERE;
                } else {
...
void open_menu_button(void) {
    ...
    for (;;) {
        ...
        case 1: {  /* MENU_OPTIONS::TALK_TO */
            ...
            if (text_ptr == 0)
                text_ptr = SNES_MSG_SYS_HANASU_NG;
            display_text_from_snes_addr(text_ptr);
...
item_action_loop:
                ...
                case 4: {
                    ...
                    if (item_entry) {
                        ...
                        display_text_from_snes_addr(help_text_addr);
                    }
                ...
                case 2: {
                    ...
                    if (give_entry && give_target != goods_char &&
                        ...
                        display_text_from_snes_addr(SNES_MSG_SYS_GOODS_NOCARRY);
                        close_window(WINDOW_TEXT_STANDARD);
                        goto item_action_loop;
                    }
                    ...
                    static const uint32_t carry_msg_addrs[10] = {
                        SNES_MSG_SYS_CARRY_SELF_ALIVE,               /* 0 */
                        SNES_MSG_SYS_CARRY_FAIL_OTHER_ALIVE_ALIVE,   /* 1 */
                        SNES_MSG_SYS_CARRY_FAIL_OTHER_ALIVE_DEAD,    /* 2 */
                        SNES_MSG_SYS_CARRY_OTHER_ALIVE_ALIVE,        /* 3 */
                        SNES_MSG_SYS_CARRY_OTHER_ALIVE_DEAD,         /* 4 */
                        SNES_MSG_SYS_CARRY_SELF_DEAD,                /* 5 */
                        SNES_MSG_SYS_CARRY_FAIL_OTHER_DEAD_ALIVE,    /* 6 */
                        SNES_MSG_SYS_CARRY_FAIL_OTHER_DEAD_DEAD,     /* 7 */
                        SNES_MSG_SYS_CARRY_OTHER_DEAD_ALIVE,         /* 8 */
                        SNES_MSG_SYS_CARRY_OTHER_DEAD_DEAD,          /* 9 */
                    };
                    ...
                    display_text_from_snes_addr(carry_msg_addrs[case_index]);
                ...
                case 3: {
                    ...
                    set_working_memory(goods_char);
                    set_argument_memory(goods_item_slot);
                    display_text_from_snes_addr(SNES_MSG_SYS_GOODS_DROP);
                    close_window(WINDOW_TEXT_STANDARD);
                    close_window(WINDOW_INVENTORY_MENU);
                    close_window(WINDOW_INVENTORY);
                    continue;  /* @MAIN_PAUSE_MENU */
        ...
        case 5: {  /* MENU_OPTIONS::CHECK */
            ...
            if (text_ptr == 0)
                text_ptr = SNES_MSG_SYS_NOPROBLEM;
            display_text_from_snes_addr(text_ptr);
            goto cleanup_and_close;
        }
