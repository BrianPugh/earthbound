...
static bool load_attract_mode_text_offsets(void) {
    if (attract_mode_offsets_loaded) return true;
...
uint16_t run_attract_mode(uint16_t scene_index) {
    ...
    if (scene_index >= 10) scene_index = 9;
     ...
     * position ert.buffer (written by update_leader_movement → sync_camera_to_entity)
     ...
     * EVENT_002 / party follower) uses an 8-bit STA that preserves the OBJECT_TICK_DISABLED
     * flag in the high byte.
     *
     * TODO: verify on real SNES hardware whether the bicycle sprite is
     * visible throughout scene 5 or only briefly at the start. If it stays
     * centered, there is an undiscovered mechanism that syncs entity 24's
     * abs position during bicycle mode. */
