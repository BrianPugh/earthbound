
UPDATE_HPPP_METER_TILES:
	BEGIN_C_FUNCTION_FAR
	STACK_RESERVE_VARS
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	END_STACK_VARS
	LDA RENDER_HPPP_WINDOWS
	AND #$00FF
	BEQL @RETURN
	LDA FRAME_COUNTER
	AND #$00FF
	AND #$0003
	STA @LOCAL09
.IF .DEFINED(JPN)
	CLC
	ADC #.LOWORD(GAME_STATE)
	TAX
	LDA a:game_state::party_members,X
.ELSE
	LDY #.LOWORD(GAME_STATE) + game_state::party_members
	LDA (@LOCAL09),Y
.ENDIF
	AND #$00FF
	BEQL @RETURN
	AND #$00FF
	CLC
	SBC #4
	JUMPGTS @RETURN
	LDY @LOCAL09
	SEP #PROC_FLAGS::INDEX8
	LDA CURRENTLY_DRAWN_HPPP_WINDOWS
	JSL ASR8_POSITIVE
	AND #$0001
	BEQL @RETURN
	LDA BATTLE_MENU_CURRENT_CHARACTER_ID
	CMP @LOCAL09
	BNE @NOT_ACTIVE_CHAR
	LDA #18
	BRA @AFTER_TILE_SELECT
@NOT_ACTIVE_CHAR:
	LDA #19
@AFTER_TILE_SELECT:
	OPTIMIZED_MULT @VIRTUAL04, 32
	CLC
	ADC #96
	STA @VIRTUAL02
	LDA GAME_STATE+game_state::player_controlled_party_count
	AND #$00FF
	OPTIMIZED_MULT @VIRTUAL04, 7
	PHA
	ASL
	PLA
	ROR
	STA @VIRTUAL04
	LDA #16
	SEC
	SBC @VIRTUAL04
	CLC
	ADC @VIRTUAL02
	INC
	INC
	INC
	STA @LOCAL08
	LDA @LOCAL09
	OPTIMIZED_MULT @VIRTUAL04, 7
	STA @VIRTUAL02
	LDA @LOCAL08
	CLC
	ADC @VIRTUAL02
	STA @LOCAL07
	ASL
	CLC
	ADC #.LOWORD(BG2_BUFFER)
	STA @VIRTUAL04
	STA @LOCAL06
	LDA @LOCAL07
	CLC
	ADC #$7C00
	STA @LOCAL07
.IF .DEFINED(JPN)
	LDA @LOCAL09
	CLC
	ADC #.LOWORD(GAME_STATE)
	REP #PROC_FLAGS::INDEX8
	TAX
	LDA a:game_state::party_members,X
.ELSE
	REP #PROC_FLAGS::INDEX8
	LDY #.LOWORD(GAME_STATE) + game_state::party_members
	LDA (@LOCAL09),Y
.ENDIF
	AND #$00FF
	DEC
	LDY #.SIZEOF(char_struct)
	JSL MULT168
	CLC
	ADC #.LOWORD(PARTY_CHARACTERS)
	STA @LOCAL05
	LDY #char_struct::current_hp_fraction
	LDA (@LOCAL05),Y
	STA @LOCAL04
	AND #$0001
	BEQL @SKIP_HP_UPDATE
	LDA @LOCAL04
	TAY
	STY @LOCAL03
	LDY #char_struct::current_hp
	LDA (@LOCAL05),Y
	TAX
	LDA @LOCAL09
	LDY @LOCAL03
	JSR FILL_CHARACTER_HP_TILE_BUFFER
	LDA @LOCAL09
	OPTIMIZED_MULT @VIRTUAL04, 24
	CLC
	ADC #.LOWORD(HPPP_WINDOW_BUFFER)
	STA @VIRTUAL02
	LDA UPLOAD_HPPP_METER_TILES
	AND #$00FF
	BNE @AFTER_HP_VRAM_SETUP
	LDA #.HIWORD(__BSS_START__)
	STA @LOCAL00
	LDA @LOCAL07
	STA @LOCAL01
	LDY @VIRTUAL02
	LDX #6
	SEP #PROC_FLAGS::ACCUM8
	LDA #0
	JSL PREPARE_VRAM_COPY_ENTRY_B
	.A16
	LDA #.HIWORD(__BSS_START__)
	STA @LOCAL00
	LDA @LOCAL07
	CLC
	ADC #32
	STA @LOCAL01
	LDA @VIRTUAL02
	CLC
	ADC #6
	TAY
	LDX #6
	SEP #PROC_FLAGS::ACCUM8
	LDA #0
	JSL PREPARE_VRAM_COPY_ENTRY_B
	.A16
@AFTER_HP_VRAM_SETUP:
	LDY @VIRTUAL02
	LDX #0
	STX @LOCAL08
	BRA @HP_TOP_LOOP_CHECK
@COPY_HP_TOP_TILE:
	LDA __BSS_START__,Y
	LDX @LOCAL06
	STX @VIRTUAL04
	STA __BSS_START__,X
	INY
	INY
	INC @VIRTUAL04
	INC @VIRTUAL04
	LDA @VIRTUAL04
	STA @LOCAL06
	LDX @LOCAL08
	INX
	STX @LOCAL08
@HP_TOP_LOOP_CHECK:
	CPX #3
	BNE @COPY_HP_TOP_TILE
	LDA @LOCAL06
	STA @VIRTUAL04
	CLC
	ADC #58
	STA @LOCAL08
	LDX #0
	STX @LOCAL03
	BRA @HP_BOTTOM_LOOP_CHECK
@COPY_HP_BOTTOM_TILE:
	TAX
	LDA __BSS_START__,Y
	STA __BSS_START__,X
	INY
	INY
	LDA @LOCAL08
	INC
	INC
	STA @LOCAL08
	LDX @LOCAL03
	INX
	STX @LOCAL03
@HP_BOTTOM_LOOP_CHECK:
	CPX #3
	BNE @COPY_HP_BOTTOM_TILE
	CLC
	ADC #58
	STA @VIRTUAL04
	STA @LOCAL06
	BRA @CHECK_PP_UPDATE
@SKIP_HP_UPDATE:
	LDA @VIRTUAL04
	CLC
	ADC #128
	STA @VIRTUAL04
	STA @LOCAL06
@CHECK_PP_UPDATE:
	LDY #char_struct::current_pp_fraction
	LDA (@LOCAL05),Y
	STA @LOCAL04
	AND #$0001
	BEQL @CHECK_UPLOAD_FLAG
	.A16
	LDA @LOCAL04
	STA @LOCAL00
	LDY #char_struct::current_pp
	LDA (@LOCAL05),Y
	TAY
	LDA @LOCAL05
	CLC
	ADC #char_struct::afflictions
	TAX
	LDA @LOCAL09
	JSR FILL_CHARACTER_PP_TILE_BUFFER
	LDA @LOCAL09
	OPTIMIZED_MULT @VIRTUAL04, 24
	CLC
	ADC #.LOWORD(HPPP_WINDOW_BUFFER) + 12
	STA @VIRTUAL02
	LDA UPLOAD_HPPP_METER_TILES
	AND #$00FF
	BNE @AFTER_PP_VRAM_SETUP
	LDA #.HIWORD(__BSS_START__)
	STA @LOCAL00
	LDA @LOCAL07
	CLC
	ADC #64
	STA @LOCAL01
	LDY @VIRTUAL02
	LDX #6
	SEP #PROC_FLAGS::ACCUM8
	LDA #0
	JSL PREPARE_VRAM_COPY_ENTRY_B
	.A16
	LDA #.HIWORD(__BSS_START__)
	STA @LOCAL00
	LDA @LOCAL07
	CLC
	ADC #96
	STA @LOCAL01
	LDA @VIRTUAL02
	CLC
	ADC #6
	TAY
	LDX #6
	SEP #PROC_FLAGS::ACCUM8
	LDA #0
	JSL PREPARE_VRAM_COPY_ENTRY_B
	.A16
@AFTER_PP_VRAM_SETUP:
	LDA @VIRTUAL02
	STA @LOCAL02
	LDX #0
	STX @LOCAL08
	BRA @PP_TOP_LOOP_CHECK
@COPY_PP_TOP_TILE:
	TAX
	LDA __BSS_START__,X
	LDX @LOCAL06
	STX @VIRTUAL04
	STA __BSS_START__,X
	LDA @LOCAL02
	INC
	INC
	STA @LOCAL02
	INC @VIRTUAL04
	INC @VIRTUAL04
	LDX @VIRTUAL04
	STX @LOCAL06
	LDX @LOCAL08
	INX
	STX @LOCAL08
@PP_TOP_LOOP_CHECK:
	CPX #3
	BNE @COPY_PP_TOP_TILE
	LDA @LOCAL06
	STA @VIRTUAL04
	CLC
	ADC #58
	TAY
	LDX #0
	STX @LOCAL08
	BRA @PP_BOTTOM_LOOP_CHECK
@COPY_PP_BOTTOM_TILE:
	LDA @LOCAL02
	TAX
	LDA __BSS_START__,X
	STA __BSS_START__,Y
	LDA @LOCAL02
	INC
	INC
	STA @LOCAL02
	INY
	INY
	LDX @LOCAL08
	INX
	STX @LOCAL08
@PP_BOTTOM_LOOP_CHECK:
	CPX #3
	BNE @COPY_PP_BOTTOM_TILE
@CHECK_UPLOAD_FLAG:
	LDA UPLOAD_HPPP_METER_TILES
	AND #$00FF
	BEQ @RETURN
	SEP #PROC_FLAGS::ACCUM8
	STZ UPLOAD_HPPP_METER_TILES
@RETURN:
	REP #PROC_FLAGS::ACCUM8 | PROC_FLAGS::INDEX8
	END_C_FUNCTION
