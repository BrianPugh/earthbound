
CHECK_ENTITY_COLLISION_AT_POSITION:
	BEGIN_C_FUNCTION_FAR
	STACK_RESERVE_VARS
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_PARAM_INT16
	STACK_RESERVE_PARAM_INT16
	STACK_RESERVE_PARAM_INT16
	STACK_RESERVE_RETURN_INT16
	END_STACK_VARS
	STY @LOCAL08
	STX @VIRTUAL02
	STX @LOCAL07
	TAY
	LDA #ENTITY_COLLISION_NO_OBJECT
	STA @LOCAL06
	LDA @LOCAL08
	ASL
	TAX
	LDA ENTITY_HITBOX_ENABLED,X
	BEQL @DONE
	LDA ENTITY_DIRECTIONS,X
	CMP #DIRECTION::RIGHT
	BEQ @SELF_FACING_HORIZONTAL
	CMP #DIRECTION::LEFT
	BNE @SELF_FACING_VERTICAL
@SELF_FACING_HORIZONTAL:
	LDA @LOCAL08
	ASL
	TAX
	LDA ENTITY_HITBOX_LEFT_RIGHT_WIDTHS,X
	STA @LOCAL05
	LDA ENTITY_HITBOX_LEFT_RIGHT_HEIGHTS,X
	STA @VIRTUAL04
	BRA @COMPUTE_BOUNDS
@SELF_FACING_VERTICAL:
	LDA ENTITY_HITBOX_UP_DOWN_WIDTHS,X
	STA @LOCAL05
	LDA ENTITY_HITBOX_UP_DOWN_HEIGHTS,X
	STA @VIRTUAL04
@COMPUTE_BOUNDS:
	LDA @LOCAL05
	STA @VIRTUAL02
	TYA
	SEC
	SBC @VIRTUAL02
	STA @LOCAL04
	LDA @LOCAL05
	ASL
	STA @LOCAL03
	LDA @LOCAL07
	STA @VIRTUAL02
	SEC
	SBC @VIRTUAL04
	STA @LOCAL07
	LDA #0
	STA @VIRTUAL02
	STA @LOCAL02
	JMP @CHECK_LOOP_CONDITION
@CHECK_ENTITY:
	LDA @VIRTUAL02
	CMP @LOCAL08
	BEQL @NEXT_ENTITY
	LDA @VIRTUAL02
	CMP #23
	BEQL @NEXT_ENTITY
	LDA @VIRTUAL02
	ASL
	TAX
	LDA ENTITY_SCRIPT_TABLE,X
	CMP #.LOWORD(-1)
	BEQL @NEXT_ENTITY
	LDA ENTITY_COLLIDED_OBJECTS,X
	CMP #ENTITY_COLLISION_DISABLED
	BEQ @NEXT_ENTITY
	LDA ENTITY_HITBOX_ENABLED,X
	BEQ @NEXT_ENTITY
	LDA ENTITY_DIRECTIONS,X
	CMP #DIRECTION::RIGHT
	BEQ @OTHER_FACING_HORIZONTAL
	CMP #DIRECTION::LEFT
	BNE @OTHER_FACING_VERTICAL
@OTHER_FACING_HORIZONTAL:
	LDA @VIRTUAL02
	ASL
	TAX
	LDY ENTITY_HITBOX_LEFT_RIGHT_WIDTHS,X
	LDA ENTITY_HITBOX_LEFT_RIGHT_HEIGHTS,X
	STA @LOCAL01
	BRA @TEST_OVERLAP
@OTHER_FACING_VERTICAL:
	LDY ENTITY_HITBOX_UP_DOWN_WIDTHS,X
	LDA ENTITY_HITBOX_UP_DOWN_HEIGHTS,X
	STA @LOCAL01
@TEST_OVERLAP:
	LDA @VIRTUAL02
	ASL
	TAX
	LDA @LOCAL01
	STA @VIRTUAL02
	LDA ENTITY_ABS_Y_TABLE,X
	SEC
	SBC @VIRTUAL02
	STA @LOCAL00
	SEC
	SBC @VIRTUAL04
	CMP @LOCAL07
	BCS @NEXT_ENTITY
	LDA @LOCAL01
	CLC
	ADC @LOCAL00
	CMP @LOCAL07
	BLTEQ @NEXT_ENTITY
	STY @VIRTUAL02
	LDA ENTITY_ABS_X_TABLE,X
	SEC
	SBC @VIRTUAL02
	STA @LOCAL00
	TYA
	ASL
	TAX
	LDA @LOCAL00
	SEC
	SBC @LOCAL03
	CMP @LOCAL04
	BCS @NEXT_ENTITY
	STX @VIRTUAL02
	LDA @LOCAL00
	CLC
	ADC @VIRTUAL02
	CMP @LOCAL04
	BLTEQ @NEXT_ENTITY
	LDA @LOCAL02
	STA @VIRTUAL02
	STA @LOCAL06
	BRA @DONE
@NEXT_ENTITY:
	LDA @LOCAL02
	STA @VIRTUAL02
	INC @VIRTUAL02
	LDA @VIRTUAL02
	STA @LOCAL02
@CHECK_LOOP_CONDITION:
	LDA @VIRTUAL02
	CMP #30
	BNEL @CHECK_ENTITY
@DONE:
	LDA @LOCAL08
	ASL
	TAX
	LDA @LOCAL06
	STA ENTITY_COLLIDED_OBJECTS,X
	LDA @LOCAL06
	END_C_FUNCTION
