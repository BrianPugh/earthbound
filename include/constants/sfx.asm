.ENUM SFX
	NONE = 0
	CURSOR1 = 1
	CURSOR2 = 2
	CURSOR3 = 3
	CURSOR4 = 4
	CURSOR_INVALID = 5
	UNUSED_06 = 6
	TEXT_PRINT = 7
	DOOR_OPEN = 8
	DOOR_CLOSE = 9
	PHONE_RING = 10
	PHONE_ANSWERED = 11
	CASH_REGISTER = 12
	CAMERA_SHUTTER = 13
	BUBBLE_MONKEY_FLOAT = 14
	ROBOT_FOOTSTEP = 15
	PRESENT_OPENED = 16
	FALLING = 17
	STAIRS = 18
	SHALLOW_WATER = 19
	DEEP_WATER = 20
	MAGICANT_FOOTSTEP = 21
	ONETT_TRUMPET_SOLO = 22
	BICYCLE_BELL = 23
	ALLY_ATTACKS = 24
	ENEMY_ATTACKS = 25
	ATTACK_ABOUT_TO_LAND = 26
	MENU_OPEN_CLOSE = 27
	PRAYING = 28
	PSI_USED = 29
	DAMAGE_TAKEN = 30
	SMAAAASH = 31
	ALLY_FELL = 32
	ENEMY_DEFEATED = 33
	MISSED = 34
	DODGED = 35
	RECOVER_HP = 36
	RECOVER_STATUS = 37
	SHIELD = 38
	PSI_SHIELD = 39
	STAT_INCREASE = 40
	STAT_DECREASE = 41
	PSI_HYPNOSIS = 42
	PSI_MAGNET = 43
	PSI_PARALYSIS = 44
	PSI_BRAINSHOCK = 45
	TOOK_DAMAGE = 46
	TOOK_MORTAL_DAMAGE = 47
	PSI_ROCKIN_1 = 48
	PSI_ROCKIN_2 = 49
	PSI_ROCKIN_3 = 50
	PSI_FIRE = 51
	PSI_FIRE_2 = 52
	PSI_FIRE_3 = 53
	COUNTER_PSI = 54
	ENEMY_PSI_USED = 55
	PSI_FREEZE_1 = 56
	PSI_FREEZE_2 = 57
	PSI_FREEZE_FINISH = 58
	HP_SUCKER = 59
	PSI_THUNDER_MISS = 60
	PSI_THUNDER_HIT = 61
	PSI_THUNDER_MISS_2 = 62
	PSI_THUNDER_DAMAGE = 63
	PSI_STARSTORM = 64
	PSI_FLASH_1 = 65
	PSI_FLASH_2 = 66
	PSI_FLASH_3 = 67
	FOOD_EATEN = 68
	UNUSED_45 = 69
	BOTTLE_ROCKET_LAUNCHED = 70
	ITEM_SPRAY = 71
	CALLED_FOR_HELP = 72
	REFLECT_DAMAGE = 73
	DEVIL_MACHINE_OFF = 74
	FIRE_BREATH = 75
	PSI_DEFENSE_DOWN = 76
	BEAM = 77
	BELCH = 78
	PSI_PARALYSIS_O = 79
	PSI_BRAINSHOCK_O = 80
	SPRAY = 81
	SPORES = 82
	AFFLICTED = 83
	OUCH = 84
	MYSTERIOUS_ACTION = 85
	SUMMONED_STORM = 86
	UNUSED_57 = 87
	EXTINGUISHING_BLAST = 88
	REFILLED = 89
	DEFENSES_DESTROYED = 90
	PSI_FIRE_4 = 91
	MINI_BARF = 92
	PSI_REFLECTED = 93
	NAMING_CONFIRM = 94
	MAGIC_BUTTERFLY = 95
	GHOST = 96
	CUTSCENE_STAIRS = 97
	EXCLAIM = 98
	SHIELD_KILLER = 99
	MYSTERIOUS_TRANSPORT = 100
	CHEEP = 101
	CONGRATS = 102
	LEARNED_PSI = 103
	CLUCK = 104
	PYRAMID_1 = 105
	PYRAMID_2 = 106
	PYRAMID_3 = 107
	PYRAMID_4 = 108
	PYRAMID_5 = 109
	PYRAMID_OPENED = 110
	RAPID_KNOCKING = 111
	AREA_TRANSITION = 112
	MYSTERIOUS = 113
	MYSTERIOUS_2 = 114
	EQUIPPED_ITEM = 115
	TOOK_ITEM = 116
	OPENED_BOX_2 = 117
	GOT_ITEM = 118
	OPENED_LOCKED_DOOR = 119
	ITEM_SOLD = 120
	TENDA_POWER = 121
	TEXT_INPUT = 122
	CURSOR1_COPY = 123
	NAMING_CURSOR_VERTICAL = 124
	NAMING_BACKSPACE = 125
	NAMING_CONFIRM_ALT = 126
	UNUSED_7F = 127
.ENDENUM
